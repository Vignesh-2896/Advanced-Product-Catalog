...
{
	let parameter_id;
	let typeArray = ['Games', 'Books', 'Electronics', 'Snacks & Drinks', 'Fruits & Veggies'];
...
{
	let similar_array = [];
	let prod_type = document.getElementById("type_" + parameter_id).innerHTML;
	let prod_price = document.getElementById("price_" + parameter_id).innerHTML;
	let promise = Promise.resolve(parameter_id);
	promise.then(function (result) {
		return Promise.all([similar_price(prod_price), similar_type(prod_type)]);	//Running multiple promises at a time
	}).then(function (result) {
		let price_similar = result[0];												//Array that contains similar products based on Price
		let type_similar = result[1];												//Array that contains similar products based on Type
